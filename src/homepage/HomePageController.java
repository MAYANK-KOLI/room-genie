...
public class HomePageController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
      ...
      //  usernameLabel.setText(login.LoginController.user.getUsername());
        ...
        check_in_Buttton.sceneProperty().addListener((observable, oldScene, newScene) -> {
            if (newScene != null) {
                // Now that the scene is not null, bind the button's width and height to the scene's size
                check_in_Buttton.prefWidthProperty().bind(newScene.widthProperty().multiply(0.3));
                check_in_Buttton.prefHeightProperty().bind(newScene.heightProperty().multiply(0.1));
                check_out_Buttton.prefWidthProperty().bind(newScene.widthProperty().multiply(0.3));
                check_out_Buttton.prefHeightProperty().bind(newScene.heightProperty().multiply(0.1));
                room_booking_Buttton.prefWidthProperty().bind(newScene.widthProperty().multiply(0.3));
                room_booking_Buttton.prefHeightProperty().bind(newScene.heightProperty().multiply(0.1));
                cancel_booking_Buttton.prefWidthProperty().bind(newScene.widthProperty().multiply(0.3));
                cancel_booking_Buttton.prefHeightProperty().bind(newScene.heightProperty().multiply(0.1));
                roomBTN.prefWidthProperty().bind(newScene.widthProperty().multiply(0.3));
                roomBTN.prefHeightProperty().bind(newScene.heightProperty().multiply(0.1));
                guestsBTN.prefWidthProperty().bind(newScene.widthProperty().multiply(0.3));
                guestsBTN.prefHeightProperty().bind(newScene.heightProperty().multiply(0.1));
            }
        });
    }
